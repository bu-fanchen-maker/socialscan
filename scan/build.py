...
def build(cards, meta, root=".", method=None, day=None):
    day = day or today()
    ...
    save_json(f"{root}/data/index.json", {"days": days, "meta": meta, "method": method or {}})
    t = open(f"{root}/web/template.html", encoding="utf-8").read()
    html = (t.replace("/*CARDS*/[]", json.dumps(cards, ensure_ascii=False).replace("</", "<\\/"))
             .replace("/*META*/{}", json.dumps(meta, ensure_ascii=False))
             .replace("/*METHOD*/{}", json.dumps(method or {}, ensure_ascii=False)))
