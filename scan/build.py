...
def build(cards, meta, root="."):
    ...
    days = sorted({os.path.basename(p)[:-5] for p in glob.glob(f"{root}/data/*.json") if not p.endswith("index.json")}, reverse=True)
    ...
    for p in glob.glob(f"{root}/data/*.json"): shutil.copy(p, f"{root}/site/data/")
